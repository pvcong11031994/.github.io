...
public class HttpPostRegular extends AsyncTask<String, String, String> {
     ...
     * @param c context
     */
    public HttpPostRegular(Context c) {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            ...
            RegularbookModel regularbookModel = new RegularbookModel(true, db);
            ...
            try {
                ...
                while (jsonReader.hasNext()) {
                    regularbooks = gson.fromJson(jsonReader, Regularbooks.class);
                    listValue.add(regularbooks.getLocation_id());
                    listValue.add(regularbooks.getLarge_classifications_id());
                    listValue.add(regularbooks.getLarge_classifications_name());
                    listValue.add(regularbooks.getName());
                    listValue.add(regularbooks.getPublisher_id());
                    listValue.add(regularbooks.getPublisher_name());
                    listValue.add(regularbooks.getPublish_date());
                    listValue.add(regularbooks.getJan_code());
                    listValue.add(String.valueOf(regularbooks.getInventory_number()));
                    listValue.add(String.valueOf(regularbooks.getRanking()));
                    index++;
                    indexListString += Constants.VALUE_COUNT_COLUMN_TABLE_REGULAR_BOOK_INSERT; //10: count column table regular books insert
                    if (indexListString == 990) { // 990: Maximum record import multi (1000 - 1000 % 25)
                        regularbookModel.insertData(db, indexListString, listValue);
                        listValue.clear();
                        indexListString = 0;
                    }
                }
                ...
                //Insert data < 990
                if (indexListString >= Constants.VALUE_COUNT_COLUMN_TABLE_REGULAR_BOOK_INSERT) {
                    regularbookModel.insertData(db, indexListString, listValue);
                }
    ...
    private JSONObject setParams(String... params) {
        ...
        try {
            switch (params[0]) {
                case Config.CODE_GET_LIST_REGULAR:
