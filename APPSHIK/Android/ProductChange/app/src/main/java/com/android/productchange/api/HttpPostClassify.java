...
/**
 * <h1>Http Post Classify</h1>
 * <p>
 * Task http Post to API get list Classify data
 *
 * @author tien-lv
 * @since 2017-12-18
 */

public class HttpPostClassify extends AsyncTask<String, String, String> {
     ...
     * @param c context
     */
    public HttpPostClassify(Context c) {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            ...
            CLPModel clpModel = new CLPModel(true, db);
            CLP clp;
            // begin transaction
            db.beginTransaction();
            ...
            try {
                ...
                while (jsonReader.hasNext()) {
                    clp = gson.fromJson(jsonReader, CLP.class);
                    listValue.add(clp.getId());
                    listValue.add(clp.getName());
                    index++;
                    indexListString += Constants.VALUE_COUNT_COLUMN_TABLE_CLASSIFY_INSERT; //2: count column table period books insert
                    if (indexListString == 998) { // 998: Maximum record import multi (1000 - 1000 % 25)
                        clpModel.insertData(db, indexListString, listValue);
                        listValue.clear();
                        indexListString = 0;
                    }
                }
                ...
                //Insert data < 998
                if (indexListString >= Constants.VALUE_COUNT_COLUMN_TABLE_CLASSIFY_INSERT) {
                    clpModel.insertData(db, indexListString, listValue);
                }
    ...
    private JSONObject setParams(String... params) {
        ...
        try {
            switch (params[0]) {
                case Config.CODE_GET_LIST_CLASSIFY:
