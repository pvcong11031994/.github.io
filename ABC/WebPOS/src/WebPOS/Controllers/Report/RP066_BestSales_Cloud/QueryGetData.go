...
`
	...
	// ASO-5754 [BA]mBAWEB-v16a 売上ベスト：商品名などをCloudSQLから取得 - ADD START
...
`
	...
	// ASO-5843 MariaDBをCLOUDSQLに引っ越して、Mariaを停止する - EDIT START
	//rows, err = dbCloud.Query(sqlStringMJan, argsMJan...)
	...
	// ASO-5754 [BA]mBAWEB-v16a 売上ベスト：商品名などをCloudSQLから取得 - ADD END
	...
	for i, _ := range salesItem {
		...
		// ASO-5754 [BA]mBAWEB-v16a 売上ベスト：商品名などをCloudSQLから取得 - ADD START
		if mJanData, ok := listMJan[salesItem[i].JanCd]; ok {
			salesItem[i].ProductName = mJanData.ProductName
			salesItem[i].AuthorName = mJanData.AuthorName
			salesItem[i].MakerName = mJanData.MakerName
			salesItem[i].SellingDate = mJanData.SellingDate
			salesItem[i].SalesTaxExcUnitPrice = float64(mJanData.ListPrice)
		}
		// ASO-5754 [BA]mBAWEB-v16a 売上ベスト：商品名などをCloudSQLから取得 - ADD END
		dataHaveRankNo := []interface{}{}
...
func queryGetDataWithJobId(ctx *gf.Context, sql string, form QueryForm, randStringFromSQL string, exCols []string) (*RpData, error) {
	...
	for {
		...
		rs.SalesBodyQuantity = row.ValueMap["sales_body_quantity"].Int()
		if form.ControlType == CONTROL_TYPE_BOOK {
			// ASO-5754 [BA]mBAWEB-v16a 売上ベスト：商品名などをCloudSQLから取得 - START END
			//rs.AuthorName = row.ValueMap["author_name"].String()
			// ASO-5754 [BA]mBAWEB-v16a 売上ベスト：商品名などをCloudSQLから取得 - DEL END
		} else if form.ControlType == CONTROL_TYPE_MAGAZINE {
			rs.MagazineCode = row.ValueMap["magazine_code"].String()
