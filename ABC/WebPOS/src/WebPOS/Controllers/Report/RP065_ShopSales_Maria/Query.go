...
func Query(ctx *gf.Context) {
	...
	if len(form.ShopCd) == 0 {
		...
		ctx.View = "report/065_shop_sales_maria/result_0.html"
		return
	}
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	sql, _, _, exCols := buildSql(form, ctx, false)
	...
	data.HeaderCols = []string{"略称", "順位", "JANコード", "品名", "著者", "出版社名", "発売日", "本体価格", "入荷累計", "売上累計", "在庫数", "初売上日", "期間売上合計"}
	...
	if form.SearchHandleType == RPComon.REPORT_SEARCH_TYPE_HANDLE_CSV {
		/* --------------------------------------------------------
		----------------------------------------------------------*/
		data.HeaderCols = []string{"共有書店コード", "店舗コード", "店舗名", "JANコード", "品名", "著者", "出版社名", "発売日", "本体価格", "入荷累計", "売上累計", "在庫数", "初売上日", "期間売上合計"}
		file := ""
		shortFile := ""
	...
	if data.CountResultRows == 0 {
		...
		ctx.View = "report/065_shop_sales_maria/result_4.html"
	}
}
