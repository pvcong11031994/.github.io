...
func Query(ctx *gf.Context) {
	...
	if err != nil || strings.Compare(messageErr, "") != 0 {
		...
		ctx.View = "report/057_single_goods_cumulative/result_0.html"
		return
	}
	...
	//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	// Check JAN (単品推移画面)++++++++++++++++++++++++++++++++
	if form.FlagSingleItem != "" && len(form.JAN) == 0 {
	...
	if len(dataSingleItem) > 0 {
		userJan.ProductName = dataSingleItem[0].GoodsName
		userJan.MakerName = dataSingleItem[0].PublisherName
		userJan.AuthorName = dataSingleItem[0].AuthorName
		userJan.UnitPrice = strconv.FormatInt(dataSingleItem[0].Price, 10)
		userJan.ReleaseDate = dataSingleItem[0].SaleDate
		userJan.Memo = Common.INIT_MEMO
	}
	...
	ctx.ViewData["dataLen"] = len(dataSingleItem) - 1
	...
	if len(dataSingleItem) == 0 {
		...
		ctx.View = "report/057_single_goods_cumulative/single_item_transition_result.html"
...
func SetDataGraph(ctx *gf.Context, dataSingleItem SingleItem, groupType string, listRange []ModelItems.MasterCalendarItem) {
	...
	for _, col := range listRange {
		...
		listResultGraph = append(listResultGraph, SingleItemCumulativeResultItem{
			...
			TooltipReceivingQuantity: toolTipItem + " 期間累計（入荷）:" + strconv.FormatInt(valueReceivingQuantity, 10) + "冊",
			ValueSalesQuantityDay:    valueSalesQuantity,
			TooltipSalesQuantityDay:  toolTipItem + " 期間累計（売上）:" + strconv.FormatInt(valueSalesQuantity, 10) + "冊",
		})
