...
--	)
	...
	SELECT
		...
		--SUM(SAFE_CAST(IF(bqsl.received_datetime < bqsl_2.received_datetime,0, bqsl.cumulative_receiving_quantity) AS INT64)) AS stok_cumulative_receiving_quantity,
		--SUM(SAFE_CAST(IF(bqsl.received_datetime < bqsl_2.received_datetime,0, bqsl.cumulative_sales_quantity) AS INT64)) AS stok_cumulative_sales_quantity,
		...
		--SUM(SAFE_CAST(IF(bqsl.received_datetime < bqsl_2.received_datetime,0, bqsl.stock_quantity) AS INT64)) AS stok_stock_quantity
		` + sqlRange + `,
		--MAX(bqsl.shop_seq_number) AS shop_seq_number
		--MAX(sm.shm_shared_book_store_code) AS shared_book_store_code
		MAX(sm.shm_shared_book_store_code) AS shared_book_store_code,
...
--	JOIN bqsl_2
--		ON bqsl.shop_code = bqsl_2.shop_code
--		AND bqsl.jan_code = bqsl_2.jan_code
	` + sqlJoinMasterCalendar + `
	GROUP BY
		shop_code,
		jan_code,
