...
func ResetTag() {
	defaultValidator.ResetTag()
}
func SetCase(tagCase string) {
	defaultValidator.SetTag(tagCase)
}
func ParseErr(msg string) string {
	return defaultValidator.ParseErr(msg)
}
...
func (mv *Validator) ResetTag() {
	mv.tagName = "validate"
}
func (mv *Validator) ParseErr(msg string) string {
...
func (mv *Validator) SetCase(tagCase string) {
	mv.tagCase = tagCase
}
...
func (mv *Validator) Validate(v interface{}) error {
	...
	for i := 0; i < nfields; i++ {
		...
		if tag != "" {
			var validErr error
			//
			require := st.Field(i).Tag.Get("require")
			if require != "" && nonzero(f.Interface(), require) == nil {
				validErr = mv.Valid(f.Interface(), require)
			}
			//
			tagCase := st.Field(i).Tag.Get("tagCase")
			if tagCase != "" && mv.tagCase != "" {
				if !strings.Contains(tagCase, mv.tagCase) && validErr == nil {
					continue
				}
			}
			if validErr == nil {
				validErr = mv.Valid(f.Interface(), tag)
			}
			// Generate error
			if errorList, ok := validErr.(ErrorArray); ok {
				errs = errorList
			} else {
				if validErr != nil {
					errs = ErrorArray{validErr}
				}
			}
		}
		...
		if len(errs) > 0 {
			message := st.Field(i).Tag.Get("message")
			if message != "" {
				errs = ErrorArray{errors.New(message)}
			}
